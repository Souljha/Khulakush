      ...
      <div className="max-w-2xl mx-auto">
        ...
        {cart.length === 0 ? (
          ...
          <div className="space-y-4">
            {cart.map(item => (
              <div key={item.id} className={`bg-white p-4 rounded-lg shadow-md border border-[${KHULA_KUSH_SURFACE_LIGHTER}] flex items-center justify-between`}>
                ...
                <div className="flex items-center space-x-3">
                  <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className={`p-1 rounded-full bg-gray-200 hover:bg-gray-300 text-gray-700`} aria-label={`Decrease quantity of ${item.name}`}>
                    <MinusIcon className="w-4 h-4" />
                  </button>
                  <span className="w-6 text-center">{item.quantity}</span>
                  <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className={`p-1 rounded-full bg-gray-200 hover:bg-gray-300 text-gray-700`} aria-label={`Increase quantity of ${item.name}`}>
                    <PlusIcon className="w-4 h-4" />
                  </button>
                  <button onClick={() => removeFromCart(item.id)} className={`text-red-500 hover:text-red-400 ml-2`} aria-label={`Remove ${item.name} from cart`}>
